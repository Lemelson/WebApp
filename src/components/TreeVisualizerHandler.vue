<template>
  <div
    ...
    class="tree-container flex"
  >
    <div class="title">
      <h1 class="name">
        Структура данных: {{ title }}
      </h1>
    </div>
    ...
    <h1 :class="{ hidden: Found }">
      Число {{ searchValueData }} не найдено
    </h1>
    ...
    />
    <div class="arrow-button-container mx-auto my-2 flex">
...
export default {
  ...
  methods: {
    ...
    changeCurrentFrame(event) {
      ...
      if (
        this.currentFrame
        > this.sequencesList[this.currentSequenceNumber].frames.length - 1
      ) {
